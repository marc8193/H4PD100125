...
#define __TWI_H__
  ...
  // -------------------------------------------------------------------------------------
  #if defined (__AVR_ATmega16__) || defined (__AVR_ATmega8__) || defined (__AVR_ATmega328P__) || defined (__AVR_ATmega2560__) //LTPE
  ...
  // (1 << TWSTA) - TWI Start
  #define TWI_START()                   { TWI_TWCR = (1 << TWEN) | (1 << TWINT) | (1 << TWSTA) | (1 << TWIE); } // LTPE
  ...
  #define TWI_WAIT_TILL_TWINT_IS_SET()  { \
	                                      ...
	                                      while ((!(TWI_TWCR & (1 << TWINT)) && (Counter < 0x01FF))); \
										}
										
#define SizeOfTypedCharactersInArray(arr) { \
uint16_t ArrayPointer = 0; \
while (arr[ArrayPointer] != '\0') \
{ \
	ArrayPointer++; \
} \
ArrayPointer; \
